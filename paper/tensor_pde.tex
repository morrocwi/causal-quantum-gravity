...
self-interaction potential, $J$ a source, and $\eta$ a readout/back-reaction term.
...
\item[] \textbf{Unitary oscillation.} With $D\to 0$, \eqref{eq:spine} is the wave/quantum
  channel; the $M\,\partial_{tt}$ term drives oscillation (\code{InfoSchrodinger}). \Thq{}
...
\item[] \textbf{Telegraph union.} The full \eqref{eq:spine} \emph{is} the telegraph
  equation, with the exact rational mass identity $M=1/(2\tau_c)$ (\code{InfoTelegraph}).
  \Thq{}
\end{itemize}
...
window action. Over one node-window $p\to m\to q$,
...
curvature calculus, division-free and exact over $\mathbb{Q}$. Every step is \Thq{}.
...
\paragraph{2. Riemann as second difference.}
...
upper-unipotent group with polynomial (division-free) inverse, $R_{xy}$ is the center of
$[X,Y]=a\cdot b$, with $[Y,X]=-a\cdot b$ ($kl$-antisymmetry, exact in the nilpotent center);
...
first Bianchi identity (\code{asym\_is\_antisymmetric}, \code{commutator\_is\_asym\_cross},
\code{curvature\_ij\_antisymmetric}, \code{bianchi\_jacobi}).

\paragraph{5. Second (differential) Bianchi.} On the 3-D lattice, abelian $dF=0$:
...
Cayley/Pythagorean transform $N^{\!\top}N=(1+t^2)^2 I$; a rational $SO(3)$ capstone (two
...
\end{equation}
by Gershgorin --- the largest $\LR$-eigenvalue is capped by twice the maximum node degree.
Atomicity is not a decorative postulate here: it is exactly the hypothesis $\lambda\le\Lambda$.
...
The continuum quantum speed limit (Mandelstam--Tamm / Margolus--Levitin) reads
...
\item \textbf{Solid-state phonon readout --- the Debye ceiling.} A lattice has a maximum
  phonon frequency $\omega_D\sim 10^{13}$--$10^{14}$~rad\,s$^{-1}$; that is a literal
...
\item \textbf{Margolus--Levitin saturation.} Systems engineered to saturate the ML bound
  $\tau_\perp=\pi\hbar/2E$ (maximal parallelism) are the sharpest probe: continuum ML
...
grows a Lorentzian manifold as the continuum approximation of a locally finite partial
order; Regge calculus and its dynamical-triangulation descendants (Euclidean DT and Causal
Dynamical Triangulations) discretize the Einstein--Hilbert action on simplicial
complexes and recover smooth geometry in a scaling limit; loop quantum gravity quantizes
geometry into discrete spin-network area and volume spectra; quantum graphity and related
...
(the cellular-automaton interpretation of quantum mechanics; the hypergraph rewriting
approach) reconstruct both quantum amplitudes and gravitational curvature from local
update rules on a discrete substrate. We share these programmes' starting intuition ---
...
four-colour and Feit--Thompson (odd-order) theorems, the formal proof of the Kepler
conjecture, and the formalization of relativistic kinematics and Minkowski spacetime in
Isabelle/HOL, alongside verified numerical-analysis and PDE developments --- have
...
field equations as an equation of state rather than a fundamental law: the
Bekenstein--Hawking horizon entropy; the derivation of the Einstein equations from the
Clausius relation across local horizons; the entropic-force account of gravity; and the
programme reading gravitational dynamics off horizon thermodynamics and holographic
degrees of freedom. Our horizon knife-edge $\lambda_c$ (\S\ref{sec:crossover}) is
...
quantum theory from five reasonable axioms; the derivation of finite-dimensional quantum
theory from informational principles (purification and companions); the characterization
of quantum mechanics through no-signalling and no-broadcasting in a
$C^{*}$-algebraic setting; and the relational reading in which quantum states encode an
observer's information about a system. Our spine shares the conviction that the quantum
...
\begin{itemize}\itemsep2pt
\item \textbf{Discrete-gravity programmes} (causal sets, Regge/CDT, loop quantum gravity,
  quantum graphity, cellular-automaton and hypergraph rewriting): several \emph{do} join a
...
\item \textbf{Machine-checked physics/mathematics} (Isabelle/HOL Minkowski and relativistic
  kinematics; verified PDE and numerical analysis; the four-colour, odd-order, and Kepler
  formalizations): these deliver verified kernels, but on objects that are either
...
a Coq~8.20.1 kernel over exact rationals, compiled with exit status~0 and \emph{Closed under
the global context} at every \code{Print Assumptions}; the \Fin{} witnesses are exact
