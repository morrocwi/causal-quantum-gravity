...
\emph{(C1)}~the epistemic standard --- twenty Tier-0 files, $155$ kernel-closed
...
\paragraph*{Kernel additions in this revision.} Four further standalone
files accompany this revision, each axiom-free and numerically pre-verified: the
exact linearization of the cubic on-site term of the mother force
(\code{InfoCubicLinearization.v}; its coefficient shift $3g\psi^2$ carries the
attraction sign identified by the three-channel probe); the exact componentwise
update law of the field tensor (\code{InfoTensorEvolution.v}), the first
mechanized piece of the tensor-evolution gap; the screen-growth ledger
(\code{InfoCutGrowth.v}): cut increments are exact crossing counts, and the
strain admitted across a screen is bounded by the benefit granted there --- the
static half of the discrete Raychaudhuri slot; and the six-cycle Plancherel
identities (\code{InfoModeWeights.v}). Hashes and check counts appear in
...
\midrule
\multicolumn{3}{@{}l}{Totals: twenty-four Tier-0 files, $176$ closed checks; two \Tone{} files, $6$ checks} & $182$ & \\
\bottomrule
\end{tabularx}
