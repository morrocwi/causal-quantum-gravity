...
$m \le (\hbar/2c^2)\sqrt{(K/M)(4-F_{\min})}$ whose contrapositive states that
...
\end{abstract} A three-channel probe sharpens this reading decisively: extra wiring or
stiffness alone gives the \emph{wrong} (repulsive) sign; only energy loading ---
inertia --- slows local update. The attraction sign therefore lives in the
inertia channel, whose linearized core is kernel-checked
(\code{InfoCubicLinearization.v}, SHA-256 prefix \code{4df87c1dd7f531e8}, six
closed checks: a background field shifts the first-order operator by exactly
$3g\psi^2$ pointwise); time-averaging to an effective medium is the disclosed
remaining step.
...
\emph{(C1)}~the epistemic standard --- twenty-six Tier-0 files, $186$ kernel-closed checks at this revision (the CI
badge is the live count): to our knowledge the first machine-checked kernel in any discrete-gravity
...
\node[th]  (CE) at (9.3,2.9)   {$\omega$ ceiling\\C41/C56 + A--M};
...
of one kernel, and no quantum claim is smuggled through the classical one. A first weld step on this note's side is now kernel-checked: on the
six-cycle --- the largest graph whose full Laplacian eigenbasis is rational, by
this programme's own readability classification --- energy mode-weights are
exactly amplitude-squared, with exact completeness of the mode decomposition
(\code{InfoModeWeights.v}, SHA-256 prefix \code{9e97c7f6ad08d6cd}, four closed
checks); the probability weld to the CPTP layer remains open and is named as
such.
...
Courant--Friedrichs--Lewy condition \cite{cfl} in kernel form. The sharp
Anderson--Morley edge form --- $\lambda \le \deg u + \deg v$ at a witness edge,
hence $\lambda \le 4 - F_{\min}$ --- is mechanized in
\code{InfoSpectralCeilingSharp.v}.
...
non-decreasing. The sharp Anderson--Morley form is mechanized directly
(\code{InfoSpectralCeilingSharp.v}, \code{am\_forman\_ceiling}): every exact
eigenpair with a nonconstant edge difference obeys
\begin{equation}
\omega_{\mathrm{int}}^2 \;\le\; \frac{K}{M}\,\bigl(4 - F_{\min}\bigr),
\label{eq:omceil}
\end{equation}
sharpening the $2d_{\max}$ route by the factor two.
\end{theorem}
...
\boxed{\;
m \;\le\; \frac{\hbar}{2c^2}\,
\sqrt{\frac{K}{M}\,\bigl(4 - F_{\min}\bigr)}\;}
\label{eq:massceil}
\end{equation}
\end{proposition}

Numerically, over $300$ random graphs the sharp bound \eqref{eq:massceil} was
never violated --- nor was the argmax-edge Gershgorin step of its proof --- and the
scaling is tight in practice: the correlation between $\omega_{\max}^2$ and
$4-F_{\min}$ across a $200$-graph ensemble is $+0.98$.
...
relabel is exactly $\lambda_{\max} \le 4 - F_{\min}$. That sharp form is now
mechanized over exact rationals in witness form
(\code{InfoSpectralCeilingSharp.v}, SHA-256 prefix \code{1f478ed777172de3},
six closed checks) by a maximizing-edge argument that needs no edge operator;
the ceiling used throughout this note is the sharp one. Ownership is unchanged: the inequality is
Anderson--Morley's; the mechanization over $\mathbb{Q}$, the retention-monotone
composition, and the mass reading are this programme's.

\begin{corollary}[Mass forces curvature]\label{cor:forces}
If $m$ exceeds the flat-region cap
$(\hbar/2c^2)\sqrt{4K/M}$ (obtained at $F_{\min}=0$), then $F_{\min}<0$ somewhere on
...
\paragraph*{Kernel additions in this revision.} Six further standalone files
accompany this revision, each axiom-free and numerically pre-verified: the exact
linearization of the cubic on-site term (\code{InfoCubicLinearization.v}; its
coefficient shift $3g\psi^2$ carries the attraction sign identified by the
three-channel probe); the exact componentwise tensor update law
(\code{InfoTensorEvolution.v}), first mechanized piece of the tensor-evolution
gap; the screen-growth ledger (\code{InfoCutGrowth.v}), the static half of the
discrete Raychaudhuri slot; the six-cycle Plancherel identities
(\code{InfoModeWeights.v}); the strain face of back-reaction
(\code{InfoBackReaction.v}) --- one joint of the feedback loop, with the two
remaining joints named Open in its header; and the exact-period homogenization
of the coefficient shift (\code{InfoShiftAverage.v}), which instances one of
those joints at the crystallographic periods. Hashes and check counts appear in
...
\code{InfoSpectralCeilingSharp.v} & --- & Anderson--Morley edge ceiling; sharp mass cap & 6 & \code{1f478ed777172de3}\\
\code{InfoCubicLinearization.v} & --- & cubic-force linearization; coefficient shift $3g\psi^2$ & 6 & \code{4df87c1dd7f531e8}\\
\code{InfoTensorEvolution.v} & --- & exact componentwise tensor update law & 4 & \code{b5638ddf507fa5b2}\\
\code{InfoCutGrowth.v} & --- & screen-growth ledger; priced cut increment & 5 & \code{4d46a4374296f375}\\
\code{InfoModeWeights.v} & --- & $C_6$ Plancherel: quadratic mode weights, completeness & 4 & \code{9e97c7f6ad08d6cd}\\
\code{InfoBackReaction.v} & --- & strain face of back-reaction; budget ledger, shift iff & 5 & \code{369e17320544e147}\\
\code{InfoShiftAverage.v} & --- & exact-period homogenization of the coefficient shift & 5 & \code{a9e3890bbdb91afe}\\
\midrule
\multicolumn{3}{@{}l}{Totals: twenty-six Tier-0 files, $186$ closed checks; two \Tone{} files, $6$ checks} & $192$ & \\
\bottomrule
\end{tabularx}
...
files at commit \code{49945bd71b05}; the files below \code{InfoPentagonSpectrum.v} postdate the pinned
commit; their hashes are of the delivered sources, pending synchronization. Each file is standalone; each compiled
...
artifact and reproduce every machine-checked claim in this note offline. Every
commit is additionally re-verified end-to-end by continuous integration
(\code{make verify} on GitHub Actions); the badge, not this table, is the
