...
\twocolumn[
  \begin{@twocolumnfalse}
    ...
    arbitrary dimension. All told: 24 Coq files, 123 theorems, every one
    machine-checked. Every claim below is accompanied by its tier, its exact
    source theorem, and a reproduction command.
    \end{abstract}
...
(\S\ref{sec:nd}), the reviewer attack response
...
\item \texttt{make verify} --- compiles all 24 \texttt{formal/*.v} files via
\texttt{coqc} in dependency order and runs \texttt{Print Assumptions} on every named
theorem (123 total), printing a tier report (\texttt{Th\_coqc} / \texttt{+reals} per
file).
...
gap. Every claim above carries its tier and its exact reproduction command.
