...
\twocolumn[
  \begin{@twocolumnfalse}
    ...
    supplement gives the full derivation and literature positioning. All told:
    35 Coq files, 188 theorems, every one machine-checked. Every claim below
...
The degree bound of \S\ref{sec:arealaw} ($d_{\max}\le4-F_{\min}$,
\texttt{InfoDegreeFromCurvature.v}) composes directly with the spectral
ceiling of \S\ref{sec:campaign} ($\omega^2\le(K/M)\,2d_{\max}$,
\texttt{InfoSpectralCeiling.v}) to give
\begin{equation}
\omega_{\mathrm{int}}^2 \;\le\; \frac{2K}{M}\,(4-F_{\min}),
\label{eq:massomega}
\end{equation}
...
mass bound,
\begin{equation}
m \;\le\; \frac{\hbar}{2c^2}\sqrt{\frac{2K}{M}\,(4-F_{\min})}.
...
$F_{\min}=0$ cap $(\hbar/2c^2)\sqrt{8K/M}$ forces negative curvature
...
(\S\ref{sec:masschain}), the reviewer attack response
