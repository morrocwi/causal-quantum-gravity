...
\twocolumn[
  \begin{@twocolumnfalse}
    ...
    golden-ratio-adjacent pentagon spectrum), a graph-native area law and a
    ...
    optimizer/kernel-energy identification. All told: 35 Coq files, 196
...
(7~theorems, \texttt{Th\_coqc}) proves Forman curvature is invariant under
...
\section{A Graph-Native Area Law and a Degree-Curvature Bound}
\label{sec:arealaw}
...
\texttt{InfoDegreeFromCurvature.v} (4~theorems, \texttt{Th\_coqc}) proves a
...
as an \emph{abstract} fact: it does not by itself establish that any graph
actually used elsewhere in this work contains face-diagonal edges (the
$n$-D ladder of \S\ref{sec:nd} uses axis-aligned edges only).

\texttt{InfoStrainTensorBridge.v} (12~theorems, \texttt{Th\_coqc}) connects
...
\section{Momentum Optimizer Theory Is the Kernel's Energy Theory}
\label{sec:optimizer}
...
(\S\ref{sec:algebra}), the area law and degree-curvature bound
...
theorem (196 total), printing a tier report (\texttt{Th\_coqc} / \texttt{+reals} per
file).
...
graph-native area law and a degree-curvature bound, each explicitly scoped
