...
\twocolumn[
  \begin{@twocolumnfalse}
    ...
    18 Coq files, 61 theorems, every one machine-checked. Every claim below is
...
\textbf{Causal signature (\texttt{InfoCausalSignature.v}, 7 theorems).} \S5 disclosed
...
\item \texttt{make verify} --- compiles all 17 \texttt{formal/*.v} files via
\texttt{coqc} in dependency order and runs \texttt{Print Assumptions} on every named
theorem (61 total), printing a tier report (\texttt{Th\_coqc} / \texttt{+reals} per
file).
