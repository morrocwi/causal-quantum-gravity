...
| `D` | **dissipative** — the arrow of time | **INDEPENDENT of `M` — but FORCED as a structural term since 2026-07-08 (update)**: the original verdict stands for what it tested (on a 2-node toy, the `M`-branch preserves a quadratic energy exactly and is reversible; the `D`-branch strictly *decreases* it every step — neither derivable *from the other* or from `L_R` *alone*). The seed-asymmetry thread then realized a possibility that file's own SCOPE block left open: imposing `L_R`'s own two axioms `{zero-row-sum, off-diagonal ≤ 0}` on the WHOLE asymmetric seed `R0` (not only its symmetric part) forces `D_i = deg_i(W) − λ·circ_i(ord)` exactly, proven for an arbitrary vertex count by induction. Honest caveats: the whole-seed extension of the axioms is natural but not proven unique; the *value* scale stays `[Open]` | `InfoDissipationIsIndependent_attempt.v` (independence of `M`); `InfoAsymmetricSeedTrifurcation.v` Part 7 + `InfoSeedArbitraryNForcing.v` (structural forcing) |
