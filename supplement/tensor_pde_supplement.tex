...
confirmable by recompiling the delivered Coq sources \cite{coq} --- whose
type theory is the Calculus of Constructions of Coquand and
Huet \cite{coquandhuet1988} --- under \code{coqc}~8.20.1
...
relative to its neighbours'' is the graph Laplacian \cite{laplace1799,kirchhoff1847}
\[
...
source, and $\eta$ the finite information-cost readout/back-reaction in the
sense of Landauer \cite{landauer1961} (an it-from-bit lineage \cite{wheeler1990}).
Setting $V\!\equiv\!0$, $J=\eta$ leaves
...
(wave/quantum, the Schr\"odinger \cite{schrodinger1926} / Klein--Gordon
\cite{klein1926,gordon1926} channel), $D\partial_t$ alone $\to$ dissipative
decay (diffusion), and their sum on $\LR$ $\to$ the telegraph equation
\cite{thomson1855,heaviside1893} of Step~5.
...
(position/velocity) update in the St\"ormer--Verlet lineage
\cite{stormer1907,verlet1967}
\[
\begin{aligned}
  X[n+1] &= X[n] + \dth\,V[n],\\
...
\emph{Statement.} In the Euler--Lagrange first-variation lineage
\cite{euler1744,lagrange1788}, over one node-window $p\to m\to q$ the discrete window action
\[
...
\[
  M=\frac{1}{2\tau_c}.
\]
The equation is exactly the telegraph equation of Thomson and Heaviside
\cite{thomson1855,heaviside1893}.
...
\emph{Statement.} The connection is the discrete Christoffel symbol
\cite{christoffel1869}, the first finite difference
...
\emph{Statement.} The Riemann curvature tensor \cite{riemann1868} is the second
finite difference $\code{riemann\_fd}=w(S(S\,j))-2\,w(S\,j)+w\,j$; affine
...
\emph{Statement.} Genuinely two-index curvature is the group commutator --- a matrix-mechanics
commutator \cite{bornjordan1925,heisenberg1927} and, around a plaquette, a
discrete Gauss--Bonnet holonomy \cite{gauss1828,bonnet1848} --- on the
...
Bianchi identity} \cite{bianchi1902}. \emph{Coq.} \code{InfoMetricCompatibleCurvature.v}:
...
Cayley / Pythagorean map \cite{cayley1846} $N=(I-A)\,\mathrm{adj}(I+A)$ with $N^{\!\top}N=(1+t^2)^2
...
\emph{Statement.} From a metric $g=\mathrm{diag}(1,w)$ the Levi-Civita /
Christoffel symbol \cite{christoffel1869} $\Gamma^2_{12}=\tfrac12\,dw/w$ is rational for $w\neq0$ (it needs only $g^{-1}$,
...
\emph{Statement.} The abelian differential Bianchi identity \cite{bianchi1902}
holds on the $3$D lattice, $d_zF_{xy}+d_xF_{yz}+d_yF_{zx}=0$, from mixed-difference commutation
...
% =====================================================================
% Bibliography --- same reference list as the main manuscript
% (paper/references.bib, shared verbatim).
% =====================================================================
